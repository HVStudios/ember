...
import { createBackupArchive, restoreBackupArchive } from "@/db/backup";
...
export function SettingsPage() {
  ...
  const downloadBackup = async () => {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `ember-backup-${new Date().toISOString().slice(0, 10)}.zip`; link.click(); URL.revokeObjectURL(url);
    setMessage("Säkerhetskopian har skapats.");
  };
  const importBackup = async (file?: File) => {
    if (!file) return;
    try { if (!window.confirm("Importen ersätter all lokal Ember-data. Vill du fortsätta?")) return; const result = await restoreBackupArchive(file); setMessage(`Återställt: ${result.completed} pass, ${result.runs} löppass, ${result.measurements} mätningar och ${result.photos} bilder.`); }
  ...
  return <div className="page settings-page"><header className="topbar"><BrandMark /><span className="program-version">EMBER 0.2</span></header>
    ...
    <section className="settings-group"><div className="settings-title"><Database size={18}/><div><strong>Säkerhetskopia</strong><span>ZIP med data och privata bilder</span></div></div><button onClick={downloadBackup}><Download size={18}/><div><strong>Exportera all data</strong><span>Gym, löpning, bilder och kroppsmått</span></div></button><button onClick={() => fileInput.current?.click()}><Upload size={18}/><div><strong>Återställ från fil</strong><span>Valideras innan befintlig data ersätts</span></div></button><input ref={fileInput} hidden type="file" accept="application/zip,.zip" onChange={(event) => void importBackup(event.target.files?.[0])}/></section>
    {message && <div className="settings-message" role="status">{message}</div>}<aside className="settings-info"><Info size={18}/><p>All träningsdata och alla bilder ligger lokalt. Exportera en backup innan du byter eller återställer telefonen.</p></aside></div>;
}
